const UpcomingCourse = require("../Models/UpcomingModels");

exports.createCourses = async (req, res) => {
    try {
        let path = "assets/" + req.filename;

            const Courses = new UpcomingCourse({
                name: req.body.name,
...
exports.getCoursesId = async (req, res) => {
    try {
        const Courses = await UpcomingCourse.findById(req.params.id);
        res.
            status(200).
            json({
                Courses: {
...
exports.getAllCourses = async (req, res) => {
    try {
        const Courses = await UpcomingCourse.find({});
        res.json(Courses);
    }
    catch (e) {
        console.error(e.message);
...
exports.updateCourses = async (req, res) => {
    try {
        let path = "assets/" + req.filename;
            ...
            const saveData = await UpcomingCourse.findByIdAndUpdate(req.params.id, data, options);
